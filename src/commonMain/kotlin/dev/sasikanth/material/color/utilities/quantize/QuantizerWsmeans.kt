  ...
  fun quantize(
    ...
    for (i in 0 until clusterCount) {
      ...
      for (j in 0 until clusterCount) {
        distanceToIndexMatrix[i]?.set(j, Distance())
      }
    }
    ...
    for (iteration in 0 until MAX_ITERATIONS) {
      for (i in 0 until clusterCount) {
        for (j in i + 1 until clusterCount) {
          val distance: Double = pointProvider.distance(clusters[i]!!, clusters[j]!!)
          distanceToIndexMatrix[j]?.get(i)!!.distance = distance
          distanceToIndexMatrix[j]?.get(i)!!.index = i
          distanceToIndexMatrix[i]?.get(j)!!.distance = distance
          distanceToIndexMatrix[i]?.get(j)!!.index = j
        }

        distanceToIndexMatrix[i]?.sortBy { it }
        for (j in 0 until clusterCount) {
          indexMatrix[i]!![j] = distanceToIndexMatrix[i]?.get(j)!!.index
        }
      }
      var pointsMoved = 0
      for (i in 0 until pointCount) {
        ...
        val previousDistance: Double = pointProvider.distance(point!!, previousCluster!!)
        var minimumDistance = previousDistance
        var newClusterIndex = -1
        for (j in 0 until clusterCount) {
          if (distanceToIndexMatrix[previousClusterIndex]?.get(j)!!.distance >= 4 * previousDistance) {
            continue
          }
          val distance: Double = pointProvider.distance(point, clusters[j]!!)
    ...
    for (i in 0 until clusterCount) {
      ...
      val possibleNewCluster: Int = pointProvider.toInt(clusters[i]!!)
