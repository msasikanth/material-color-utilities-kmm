 ...
 */ private constructor(
  ...
  companion object {
    ...
    fun fromXyzInViewingConditions(
      ...
      val rA: Double = sign(rD) * 400.0 * rAF / (rAF + 27.13)
      val gA: Double = sign(gD) * 400.0 * gAF / (gAF + 27.13)
      val bA: Double = sign(bD) * 400.0 * bAF / (bAF + 27.13)
      ...
      val mstar: Double = 1.0 / 0.0228 * log(1 + (0.0228 * m), base = E)
    ...
    private fun fromJchInViewingConditions(
      ...
      val jstar = (1.0 + 100.0 * 0.007) * j / (1.0 + 0.007 * j)
      val mstar: Double = 1.0 / 0.0228 * log(1 + (0.0228 * m), E)
      val astar: Double = mstar * cos(hueRadians)
      val bstar: Double = mstar * sin(hueRadians)
